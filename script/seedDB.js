...
mongoose.connnect(process.env.MONGODB_URI || "mongodb://localhost/bandsInTown");
...
db.Event.remove({})
   .then(() => db.Event.collection.insertMany(bookSeed))
   .then(data => {
       console.log(data.result.n,"New records innserted");
       process.exit(0);
   })
   .catch(err => {
